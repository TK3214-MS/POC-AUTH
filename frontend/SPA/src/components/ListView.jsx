...
export const ListView = (props) => {
  ...
  const handleCompleteTask = (id) => {
    ...
    execute(
      "PUT",
      protectedResources.apiTodoList.endpoint + `/${id}`,
      updatedTask
    ).then(() => {
  ...
  const handleAddTask = (name) => {
    ...
    execute("POST", protectedResources.apiTodoList.endpoint, newTask).then(
      (response) => {
  ...
  const handleDeleteTask = (id) => {
    execute("DELETE", protectedResources.apiTodoList.endpoint + `/${id}`).then(
      (response) => {
        if (response && response.message === "success") {
